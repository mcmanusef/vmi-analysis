...
for i in range(n):
    corr = np.where(np.logical_and(
        tot[index] >= totspace[i], tot[index] < totspace[i]+dtot))
    t_correction[corr] = means[i]

tfix = t_s-t_correction


index = np.where(np.logical_and(t_s-t_correction >
                                rtc[0], t_s-t_correction < rtc[1]))
...
plt.figure(-2)
plt.hist(tfix, bins=100, range=rtc, weights=tot_s)
plt.title("Corrected ToA, "+name)
plt.xlabel("t")
plt.ylabel("counts")
