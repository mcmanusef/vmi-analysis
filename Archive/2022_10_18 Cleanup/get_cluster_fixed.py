...
parser.add_argument('--uncombined', action='store_true',
                    help="Don't use TDC1 for both TOF and Laser Timing pulses")

parser.add_argument('--min', dest='min',
                    help="Create a file with unneccessary information removed")
...
args = parser.parse_args("Data\\xe005_e --g 1000".split())
...
if __name__ == '__main__':
    ...
    # %% Preliminary Calculations
        ...
        if not tdc_type[-1] == 1:
            ...
            pulse_times = correct_pulse_times(times[np.where(lengths > 1e3*args.cutoff)])
            tof_times = times[np.where(lengths < 1e3*args.cutoff)]
        else:
            lengths = np.diff(tdc_time)[tdc1[:-1]]
            pulse_times = correct_pulse_times(times[np.where(lengths > 1e3*args.cutoff)[:-1]])
