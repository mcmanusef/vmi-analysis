...
parser.add_argument('--etof', dest='etof', nargs=2, type=float,
                    default=[20, 40], help="range of electron ToF to include (in ns)")
parser.add_argument('--itof', dest='itof', nargs=2, type=float,
...
with matplotlib.backends.backend_pdf.PdfPages(args.output) as pdf:
    ...
    # plt.hist2d(y, x, bins=512, range=[[0, 256], [0, 256]])  # , norm=mpl.colors.LogNorm())
    plt.imshow(np.sum(h2, axis=2))
    ...
    # plt.figure(figsize=fsize)
    # plt.suptitle('Momentum VMI')
    # plt.hist2d(py, px, bins=512, range=[[-width, width],
    #                                     [-width, width]])  # , norm=mpl.colors.LogNorm())
    # plt.title("Full VMI Image (Log Scale)")
    # plt.xlabel("y")
    # plt.ylabel("x")
    # plt.tight_layout(rect=window)
