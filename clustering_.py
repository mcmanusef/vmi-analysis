...
class ClusterTest(base_pipeline.AnalysisPipeline):
    def __init__(self, fname):
        ...
        self.processes = {
            ...
            "Converter": VMIConverter(self.queues['chunk'], self.queues['pixel'], self.queues['pulses'], self.queues['etof'],
                                      self.queues['itof']).make_process(),
            "Clusterer": DBSCANClusterer(self.queues['pixel'], self.queues['clusters']).make_process(),
            "Correlator": TriggerAnalyzer(self.queues['pulses'], (self.queues['etof'], self.queues['itof'], self.queues['clusters']),
                                          self.queues['t_pulse'],
                                          (self.queues['t_etof'], self.queues['t_itof'], self.queues['t_cluster'])).make_process(),
            "Saver": SaveToH5(fname + ".h5",
                              {"etof": self.queues['t_etof'], "itof": self.queues['t_itof'], "pulse": self.queues['t_pulse'],
