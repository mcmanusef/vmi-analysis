...
class ClusterTest(base_pipeline.AnalysisPipeline):
    def __init__(self, fname):
        ...
        self.queues = {
            ...
            "t_etof": data_types.ExtendedQueue(names=("etof_corr", ("t_etof",)), dtypes=('i', ('f',))),
            "t_itof": data_types.ExtendedQueue(names=("itof_corr", ("t_itof",)), dtypes=('i', ('f',))),
            "t_pulse": data_types.ExtendedQueue(names=("t_pulse",),dtypes=('i',)),
            "t_cluster": data_types.ExtendedQueue(names=("clust_corr", ("t", "x", "y")), dtypes=('i', ('f', 'f', 'f'))),
        }
        self.processes = {
            "ChunkStream": TPXFileReader(fname, self.queues['chunk']).make_process(),
            "Converter": VMIConverter(self.queues['chunk'], self.queues['pixel'], self.queues['pulses'], self.queues['etof'], self.queues['itof']).make_process(),
            "Clusterer": DBSCANClusterer(self.queues['pixel'], self.queues['clusters']).make_process(),
            "Correlator": TriggerAnalyzer(self.queues['pulses'], (self.queues['etof'], self.queues['itof'], self.queues['clusters']), self.queues['t_pulse'], (self.queues['t_etof'], self.queues['t_itof'], self.queues['t_cluster'])).make_process(),
            "Saver": SaveToH5(fname+".h5",
                                {"etof": self.queues['t_etof'], "itof": self.queues['t_itof'], "pulse": self.queues['t_pulse'], "cluster": self.queues['t_cluster']},
