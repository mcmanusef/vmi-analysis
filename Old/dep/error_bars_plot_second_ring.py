...
def get_profiles(data, i, n, pol=0., plotting=True, blurring=0, max_p=0.6, mode='mean', label="", num=11):
    ...
    rs = np.linspace(r_peak[1] - width[1] / 2, r_peak[1] + width[1] / 2, num=num)
...
def main(files,
    ...
    for (r, dr, t, dt), l, (r_inter, _, t_inter, _), c in zip(intra_lines, labels, inter_lines, lines_colour_cycle):
        if max_error is not None:
            print(list(zip(*filter(lambda x: x[3]<np.radians(max_error),zip(r,dr,t,dt)))))
            r,dr,t,dt= tuple(map(np.asarray,zip(*(filter(lambda x: x[3]<np.radians(max_error),zip(r,dr,t,dt))))))

        t_adjust = np.unwrap((t % np.pi - t_inter[1] % np.pi) * -1 * np.sign(l), period=np.pi, discont=np.pi / 2)

        plt.errorbar(r - r_inter[1], np.degrees(t_adjust), xerr=dr * 2, yerr=dt * 180 / np.pi,
                     label=f"{l}: {label}",
    ...
    for (r, dr, t, dt), l, c in zip(inter_lines, labels, lines_colour_cycle):
        t_adjust = (np.unwrap(t, period=np.pi) - 2 * np.pi) * -1 * np.sign(l) + np.pi * int(l < 0)
        plt.errorbar(range(len(r)), np.degrees(t_adjust) % 180, xerr=dr, yerr=dt * 180 / np.pi,
                     label=f"{l}: {label}",
