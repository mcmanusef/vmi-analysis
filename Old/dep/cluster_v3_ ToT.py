...
def tot_correct(t, tot, correction):
    for ti, toti in zip(t, tot):
        try:
            yield ti-correction[toti], toti
        except KeyError:
            yield 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tot', dest='tot',
                        default='', help="The path of the ToT correction file")
    ...
    if args.tot:
        with open(args.tot) as f:
            correction = list(map(float, f.readlines()))
        t_pixel, tot = split_iter(tot_correct(t_pixel, tot, correction), 2)
