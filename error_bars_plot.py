...
import scipy.stats
from cv3_analysis import load_cv3, partition
...
def get_peak_angles(hist, r, t):
    ...
    for peak, width in itertools.compress(zip(peaks, widths), np.logical_and(proms/rhist[peaks] > 0.3, rhist[peaks]>0.1)):
...
# @njit
def get_profiles(data, i, n, pol=0):
    ...
    plt.figure(f"{pol}: {i}")
    plt.subplot(221)
    plt.hist2d(px, py, bins=b, range=[[-1, 1], [-1, 1]])
    plt.subplot(222)
...
if __name__ == '__main__':
    ...
    for name, pol in inputs:
        ...
        with Pool(4) as p:
            ...
            inter, intra = tuple(tuple(map(lambda x: tuple(zip(*x)), zip(*j))) for j in zip(*map(lambda i: get_profiles(data, i, n, pol=pol), range(n))))

        r_inter, dr_inter = zip(*((scipy.stats.circmean(np.asarray(i)%np.pi,high = np.pi), scipy.stats.circstd(np.asarray(i)%np.pi,high = np.pi)) for i in inter[0]))
        t_inter, dt_inter = zip(*((scipy.stats.circmean(np.asarray(i)%np.pi,high = np.pi), scipy.stats.circstd(np.asarray(i)%np.pi,high = np.pi)) for i in inter[1]))
        inter_lines.append(tuple(map(np.array, (r_inter, dr_inter, t_inter, dt_inter))))

        r_intra, dr_intra = zip(*((scipy.stats.circmean(np.asarray(i)%np.pi,high = np.pi), scipy.stats.circstd(np.asarray(i)%np.pi,high = np.pi)) for i in intra[0]))
        t_intra, dt_intra = zip(*((scipy.stats.circmean(np.asarray(i)%np.pi,high = np.pi), scipy.stats.circstd(np.asarray(i)%np.pi,high = np.pi)) for i in intra[1]))
        intra_lines.append(tuple(map(np.array, (r_intra, dr_intra, t_intra, dt_intra))))
# %%
    f, ax = plt.subplots(2, 1)
    plt.sca(ax[0])
    for (r, dr, t, dt), l in zip(inter_lines,labels):
        plt.errorbar(r, t % np.pi, xerr=dr, yerr=dt, label=l)
    plt.legend()
    plt.sca(ax[1])
    for (r, dr, t, dt), l in zip(intra_lines,labels):
        plt.errorbar(r, (t % np.pi-t[0] % np.pi), xerr=dr, yerr=dt, label=l)
