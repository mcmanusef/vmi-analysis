...
for d, e in [('xe002_s.mat', -0.1), ('xe014_e.mat', 0.2), ('xe011_e.mat', 0.3), ('xe012_e.mat', 0.5), ('xe013_e.mat', 0.6), ('xe003_c.mat', 0.9)]:  # , ('xe015_e.mat', -0.3), ('xe016_e.mat', -0.6)]:  # os.listdir(source):
    print(d)
    ...
    for b in np.linspace(-r[widths[index]]*width_coeff, r[widths[index]]*width_coeff, n_inner):
        ...
        angulardist, angle_edges = np.histogram(theta.flatten() % np.pi, weights=(mask*sample).flatten(), bins=100)
        mean_angle = angular_average(getmeans(angle_edges), high=np.pi, weights=angulardist**0.5) % np.pi
