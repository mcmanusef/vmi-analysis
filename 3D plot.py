...
hist, *e = np.histogramdd(
    (px, py, pz), bins=128, range=((-pmax, pmax), (-pmax, pmax), (-pmax, pmax))
)

hist_smooth = scipy.ndimage.gaussian_filter(hist, 0.75)
hist = hist_smooth
hist /= hist.max()
hist = hist**0.3
print(hist.shape)
...
data_to_plot = []
# marching cubes
for level in np.linspace(0.1, 0.9, 7):
    verts, faces, _, _ = measure.marching_cubes(
        hist, level, spacing=(2 * pmax / 1024, 2 * pmax / 1024, 2 * pmax / 1024)
    )
...
for level, verts, faces in data_to_plot:
    ...
    plotter.add_mesh(surf, color=cmap(level**1.5), opacity=level**2)
