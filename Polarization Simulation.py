...
def jones_matrix_waveplate(retardance, orientation):
    ...
    j_rot_minus = np.array([[ c,  s],
                            [-s,  c]], dtype=np.complex128)
    j_rot_plus = np.array([[ c, -s],
                           [ s,  c]], dtype=np.complex128)
    diag = np.array([[1.0, 0.0],
                     [0.0, np.exp(1j*retardance)]], dtype=np.complex128)
...
def material_refractive_index(wavelength_nm, material='BK7'):
    ...
    if material == 'BK7':
        n = lambda λ: (1 + (1.03961212 * λ**2) / (λ**2 - 0.00600069867) + (0.231792344 * λ**2) / (λ**2 - 0.0200179144) + (1.01046945 * λ**2) / (λ**2 - 103.560653))**0.5
        return n(wavelength_nm/1000)
    elif material == 'Fused Silica':
        n = lambda λ: (1 + (0.6961663 * λ**2) / (λ**2 - 0.0684043**2) + (0.4079426 * λ**2) / (λ**2 - 0.1162414**2) + (0.8974794 * λ**2) / (λ**2 - 9.896161**2))**0.5
        return n(wavelength_nm/1000)
    elif material == 'Calcite (O)':
        n = lambda λ: (1 + 0.73358749 +
                   (0.96464345 * λ**2) / (λ**2 - 1.9435203e-2) +
                   (1.82831454 * λ**2) / (λ**2 - 120))**0.5
        return n(wavelength_nm/1000)
    elif material == 'Calcite (E)':
        n = lambda λ: (1 + 0.35859695 +
                      (0.82427830 * λ**2) / (λ**2 - 1.06689543e-2) +
                      (0.14429128 * λ**2) / (λ**2 - 120))**0.5
        return n(wavelength_nm/1000)
    else:
...
def gvd_compensation_plate(E_jones, omega, wavelength_nm, delay_fs=0, long_wavelength_nm=1030.0, short_wavelength_nm=515.0, dispersive=False):
    if dispersive:
...
def zero_order_half_waveplate(E_jones, wavelength_nm, orientation_deg=0.0, center_wavelength_nm=1030.0):
    """
...
def glass_window(E_jones, omega, wavelength_nm, thickness_m, material='BK7'):
    ...
    """
    return apply_dispersion(E_jones, omega, wavelength_nm, thickness_m, material=material)
...
class TwoColorPulse:
    ...
    """
    def __init__(self, amp_long=1.0, amp_short=1.0, wl_long_nm=1030.0, wl_short_nm=515.0,
                 duration_fs=130.0, time_window_fs=2000.0, num_points=2**14, normalize=True):
        self.amp_long = amp_long
        self.amp_short = amp_short
        if normalize:
            self.amp_long, self.amp_short = self.amp_long / np.sqrt(self.amp_long ** 2 + self.amp_short ** 2), self.amp_short / np.sqrt(self.amp_long ** 2 + self.amp_short ** 2)
        ...
        # Prepare time array
        self.t = np.linspace(-0.5*self.time_window_fs, 0.5*self.time_window_fs, self.num_points)
    ...
    def _create_time_domain_pulses(self):
        ...
        E_x = self.amp_long * np.exp(-0.5 * (self.t ** 2) / (sigma_t ** 2)) * np.exp(1j * omega_1030 * self.t)
        E_y = self.amp_short * np.exp(-0.5 * (self.t ** 2) / (sigma_t ** 2)) * np.exp(1j * omega_515 * self.t)
        return E_x, E_y
...
class Simulation:
    ...
    def apply_optical_element_gvd_plate(self, delay_fs=0, long_wavelength_nm=1030.0, short_wavelength_nm=515.0, dispersive=False):
        self.Ew = gvd_compensation_plate(self.Ew, self.omega, self.wavelength_nm, delay_fs=delay_fs, long_wavelength_nm=long_wavelength_nm, short_wavelength_nm=short_wavelength_nm, dispersive=dispersive)

    def apply_optical_element_quarter_wave(self, orientation_deg=45.0):
        self.Ew = super_achromatic_quarter_waveplate(self.Ew, self.wavelength_nm, orientation_deg=orientation_deg)

    def apply_optical_element_half_wave(self, orientation_deg=0.0):
        self.Ew = zero_order_half_waveplate(self.Ew, self.wavelength_nm, orientation_deg=orientation_deg)

    def apply_optical_element_glass_window(self, thickness_m=0.001, material='BK7'):
        self.Ew = glass_window(self.Ew, self.omega, self.wavelength_nm, thickness_m, material)
...
def plot_stokes_parameters(t, Ex_t, Ey_t, wvln=1030):
    ...
    V = 2 * np.imag(Ex_t * np.conj(Ey_t))  # Imaginary part of the cross term (circular polarization)
