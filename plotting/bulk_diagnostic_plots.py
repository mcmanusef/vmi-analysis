...
def analyze_to_pdf(file, output, dead_pixels=None, center=(123.48, 131.01, 749053.403), angle=0, calibration=None,
    ...
    with PdfPages(output) as pdf:
        ...
        # center=(center[0], center[1], center[2]-mean)
        ...
        plt.text(0.1, 0.6,
                 f"Peak times:\n" + "\n".join([f"{i}: {t_edges[peak]:.2f} ns" for i, peak in enumerate(t_peak_index)]),
                 fontsize=12, transform=plt.gca().transAxes)

        tof_zoomed.add_subplot(212)
        ...
        plt.subplot(212)
        center_calc = np.mean(x), np.mean(y)
