...
def main(inputs = [('xe011_e', 0.3),('xe013_e', 0.6)],
         calibrated = False,
         wdir = r"J:\ctgroup\DATA\UCONN\VMI\VMI\20220613", to_load=100000):
    ...
    for name, pol in inputs:
        ...
        if calibrated:
            ...
            angle = wrap_between(ebp.get_pol_angle(os.path.join(wdir, fr"Ellipticity measurements\{name}_power.mat"),
                                                   os.path.join(wdir, r"Ellipticity measurements\angle.mat"))+4,-90,90)
            ...
            py, px, pz = cv3.load_cv3(os.path.join(wdir, fr"clust_v3\{name}.cv3"), pol=np.radians(angle), width=.05, to_load=to_load)
        ...
        # Add and ellipse
        ellipse = patches.Ellipse((0.9, 0.1), 0.15, 0.1, linewidth=2, edgecolor='red', facecolor='none', transform=ax.transAxes)
        ax.add_patch(ellipse)

        # Add two triangles

        left_triangle = patches.Polygon([[0.9-0.01*np.sign(pol), 0.15], [0.9+0.01*np.sign(pol), 0.16], [0.9+0.01*np.sign(pol), 0.14]], linewidth=1,
                                        edgecolor='red', facecolor='red', transform=ax.transAxes)
        ax.add_patch(left_triangle)

        right_triangle = patches.Polygon([[0.90+0.01*np.sign(pol), 0.05], [0.9-0.01*np.sign(pol), 0.06], [0.9-0.01*np.sign(pol), 0.04]], linewidth=1,
                                         edgecolor='red', facecolor='red', transform=ax.transAxes)
        ax.add_patch(right_triangle)

        ax.text(.9, .1, f"{ell:.3f}", ha='center', va='center', transform=ax.transAxes)

        rect2 = patches.Rectangle((0.6, 0.8), 0.4, 0.2, linewidth=1, edgecolor='black', facecolor='white', transform=ax.transAxes)
        ax.add_patch(rect2)
