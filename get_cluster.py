...
def get_clusters(data):
    #Runs DBSCAN Clustering on data, where data has the form (xlist, ylist)
...
if __name__ == '__main__':
    ...
    in_name=filename+'.h5'
    out_name=filename+'_cluster.h5' if args.output=='notset' else args.output
    ...
    to_keep=np.where(np.logical_xor(args.reverse,np.logical_and(1e9*args.start<=np.diff(pulse_times),np.diff(pulse_times)<=1e9*args.end)))[0]
