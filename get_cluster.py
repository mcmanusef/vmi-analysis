...
def cluster_analysis(filename,groupsize):
    ...
    # print(np.where(np.diff(toa)<0)[0])
    # print(np.where(np.diff(pulses)<0)[0])
    for i in range(num): 
    ...
    # print(num)
    # print(len(data))
    # print(len(data[35635][0]))
    # print(len(data[35636][0]))
    # print(max([len(data[i][0]) for i in range(len(data))]))
    print('Clustering:', datetime.now().strftime("%H:%M:%S"))    
    #Gather data
    
    # clusters=[get_clusters(d) for d in data]
    clusters=list(np.zeros(len(data)))
    m=len(data)
    for i in range(len(data)):
        if i%100==0:
            print('{}/{}'.format(i,m))
        clusters[i]=get_clusters(data[i])
    # for i in range(int(len(data)/groupsize)+1):
    #     temp=min((i+1)*groupsize,len(data))
    #     print(i*groupsize,temp)
    #     with Pool(16) as p:
    #         to_add=map(get_clusters,data[i*groupsize:temp])
