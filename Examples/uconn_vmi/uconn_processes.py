...
from vmi_analysis.processing.data_types import ExtendedQueue, Chunk, PixelData, TriggerTime, ToF
from vmi_analysis.processing.processes import AnalysisStep
from vmi_analysis.processing.tpx_conversion import process_chunk, apply_timewalk, toa_correction, sort_tdcs
            ...
            if len(x_rem) == 1 and len(etof_rem) == 1:
                if 0 < etof_rem[0] < 2000:
                    self.uncalibrated_pol_plane_data[
                        int(x_rem[0]),
                        np.digitize(etof_rem[0], np.linspace(0, 2000, n=256)),
                    ] -= 1
