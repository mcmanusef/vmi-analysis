...
class CV4ConverterPipeline(AnalysisPipeline):
    ...
    def __init__(self, input_path, output_path, save_pixels=False, cluster_processes=1, converter_processes=1, cluster_class=None, **kwargs):
        ...
        if converter_processes == 1:
            converter_processes = {"Converter": processes.VMIConverter(
...
class StonyBrookClusterPipeline(AnalysisPipeline):
    def __init__(self, input_path, output_path, **kwargs):
        ...
        self.processes = {
            ...
            "Correlator": processes.TriggerAnalyzer(self.queues['pulse'], (self.queues['tof'], self.queues['clusters']), self.queues['t_pulse'],
                                                    (self.queues['t_tof'], self.queues['t_cluster'])).make_process(),
            "Saver": processes.SaveToH5(output_path, {"t_tof": self.queues['t_tof'], "t_cluster": self.queues['t_cluster'],
                                                      "t_pulse": self.queues['t_pulse']}).make_process(),
        }
