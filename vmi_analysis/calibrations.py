...
def general_calibration(
    x, y, t, center, E_xy, E_t, angle=0.0, symmetrize=True, cutup=False
):
    ...
    if symmetrize:
        px, py, pz = (
            np.column_stack([px, -px]).flatten(),
            np.column_stack([py, -py]).flatten(),
            np.column_stack([pz, -pz]).flatten(),
        )
    return px, py, pz
...
def calibration_20240208(x, y, t, center, angle=0, symmetrize=True, cutup=False):
    ...
    E_t = lambda t: np.where(
        t < 0,
        0.1137238 * t**2 + 0.00291691 * t**3,
        0.03621571 * t**2 - 0.00082477 * t**3,
    )
...
def calibration_20241120(
    x, y, t, center=(133.2, 131.7, 496), angle=-0.43, symmetrize=True, cutup=False
):
    E_xy = lambda x: 0.000519 * x**2
    z = lambda t: np.where(
        t < 0,
        ...
        np.polyval(
            [0.13548162025504634, -6.827514146748386, 125.8002806866771, 0, 0], t
        )
        ** 0.5,
    )
    return general_calibration(
        x, y, t, center, E_xy, lambda t: E_xy(z(t)), angle, symmetrize, cutup
    )


def calibration_20250123(
    x, y, t, center=(127.75, 128.75, 495.42), angle=-0.5, symmetrize=True, cutup=False
):
    E_xy = lambda x: 0.000519 * x**2
    z = lambda t: np.where(
        t < 0,
        ...
        np.polyval(
            [-0.04885889138712628, -2.0134037673354435, 92.1302612227862, 0, 0], t
        )
        ** 0.5,
    )
    return general_calibration(
