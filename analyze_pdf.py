...
parser.add_argument('--out', dest='output', default="output.pdf", help="Output Filename")
parser.add_argument('--noclust',action='store_true',help="Do not analyze any clustered data.")
...
with matplotlib.backends.backend_pdf.PdfPages(args.output) as pdf:
    ...
    plt.subplot(222)
    plt.hist2d(y,x,bins=256,range=[[0,256],[0,256]])#,weights=tot_s, vmax=40000)
    ...
    index=np.where(np.logical_and(time_after>toa_int[0],time_after<toa_int[1]))
    xs=x[index]
    ys=y[index]
    ts=time_after[index]
    tots=tot[index]
    ...
    if not args.noclust:
        ...
        for i in range(n):
            corr=np.where(np.logical_and(tots>=totspace[i],tots<totspace[i]+dtot))
        ...
        plt.subplot(224)
        plt.hist2d(tfix,xs,bins=256,range=[[-toa_range/2,toa_range/2],[0,256]])
        plt.xlabel("t")
        plt.ylabel("x")
        plt.tight_layout(rect=window)
        
        plt.subplot(221)
        plt.hist2d(ys,tfix,bins=256,range=[[0,256],[-toa_range/2,toa_range/2]])
        plt.xlabel("y")
        plt.ylabel("t")
        plt.tight_layout(rect=window)
        ...
        plt.subplot(222)
        plt.title("ToA vs ToT")
        plt.hist2d(tfix, tots, bins=256, range=[[-toa_range/2,toa_range/2],rtot])
        plt.xlabel("ToA (ns)")
        plt.ylabel("ToT (ns)")
        plt.tight_layout(rect=window)
